...
class cyclic_timed_player(cyclic_player):
    def __init__(self,offset_pos,
                      offset_career,
                      offset_vocb,
                      offset_special_token,
                      offset_relative_pos,
                      num_player):
        super(cyclic_timed_player,self).__init__(offset_pos,offset_career,offset_vocb,
                                                 offset_special_token,offset_relative_pos,num_player)
    ...
    def __call__(self,one_action):
        ...
        # replace system keys
        sys1_aug = []
        for sys_t in one_action["sys1"]:
            sys1_list = sys_t.split(" ")
            for token_idx in range(len(sys1_list)):
                if sys1_list[token_idx] in self.player_pos:
                    sys1_list[token_idx] = str(shuffle_map[int(sys1_list[token_idx])])
            sys1_aug.append(" ".join(sys1_list))

        if one_action["sys2"] != None:
            sys2_aug = []
            for sys_t in one_action["sys2"]:
                sys2_list = sys_t.split(" ")
                for token_idx in range(len(sys2_list)):
                    if sys2_list[token_idx] in self.player_pos:
                        sys2_list[token_idx] = str(shuffle_map[int(sys2_list[token_idx])])
                sys2_aug.append(" ".join(sys2_list))
        else:
            sys2_aug = None
